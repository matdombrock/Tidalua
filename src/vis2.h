...
void vis_clear() {
    // Use ANSI escape codes instead of system call for better performance
    printf("\033[2J\033[H");
}
...
void vis_loop() {
    ...
    // Optimization for macOS - reduce frame rate on macOS
    #ifdef __APPLE__
    const int FRAME_DELAY_MS = 50;  // 20 FPS on macOS
    #else
    const int FRAME_DELAY_MS = 33;  // 30 FPS on Linux
    #endif
    
    // Cache timestamp of last render to limit framerate
    ...
    while (run) {
        ...
        long time_diff_ms = (current_time.tv_sec - last_render_time.tv_sec) * 1000 + 
                           (current_time.tv_nsec - last_render_time.tv_nsec) / 1000000;
        
        if (time_diff_ms >= FRAME_DELAY_MS) {
        ...
        // Check for input (this is lightweight)
        run = vis_input();
        
        // Use a short sleep to reduce CPU usage but maintain responsiveness
        usleep(5 * 1000);
    }
