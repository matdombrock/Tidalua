...
// Fast sine approximation for better performance
static inline float fast_sin(float x) {
    // Convert input to range -1 to 1
    float y = x / (float)M_PI;
    
    // Range reduce to -1..1
    y = y - floorf(y + 0.5f) * 2.0f;
    
    // 7th-order Chebyshev polynomial approximation
    const float A = 4.0f / (float)M_PI;
    const float B = -4.0f / ((float)M_PI * (float)M_PI);
    return A * y + B * y * fabsf(y);
}

static inline float synth_get_sample(float phase, int osc) {
    int mode = _synth[osc].wave % 6; // Wrap wave mode
    float sample = 0.0f;
    
    // Normalize phase to 0-2PI
    float phase_normalized = phase;
    while (phase_normalized >= 2.0f * M_PI) {
        phase_normalized -= 2.0f * M_PI;
    }
    
    // Precompute phase/(2*PI) for saw and triangle to avoid repeated division
    float phase_normalized_div_2pi = phase_normalized / (2.0f * M_PI);
    
    // Use a branch-free approach for better performance on modern processors
    switch (mode) {
        case 0: // Off
            sample = 0.0f;
            break;
        case 1: // Sine
            #ifdef __APPLE__
            sample = fast_sin(phase_normalized); // Use fast sine on macOS
            #else
            sample = sinf(phase_normalized);
            #endif
            break;
        case 2: // Square
            sample = phase_normalized < M_PI ? 1.0f : -1.0f;
            break;
        case 3: // Saw
            sample = 2.0f * phase_normalized_div_2pi - 1.0f;
            break;
        case 4: // Triangle
            sample = 2.0f * fabsf(2.0f * phase_normalized_div_2pi - 1.0f) - 1.0f;
            break;
        case 5: // Noise
            // Use a faster random number generator on macOS
            #ifdef __APPLE__
            sample = 2.0f * ((float)(rand() & 0x7FFF) / (float)0x7FFF) - 1.0f;
            #else
    ...
    // Apply envelope if enabled - direct multiplication is faster than branch
    if (_synth[osc].env_enabled) {
        sample *= synth_get_env(osc);
    }
...
// Make filter buffers static to reduce cache misses
static float _synth_lowpass_buf_in[2 + OSC_COUNT] = {0};
static float _synth_lowpass_buf_out[2 + OSC_COUNT] = {0};

// Use inline for filter functions to help compiler optimize
static inline float synth_lowpass(int index, float input, float cutoff, float dt) {
    float prev_input = _synth_lowpass_buf_in[index];
    float prev_output = _synth_lowpass_buf_out[index];

    // Precompute filter constants
    static float cached_cutoff[2 + OSC_COUNT] = {0.0f};
    static float cached_dt[2 + OSC_COUNT] = {0.0f};
    static float cached_alpha[2 + OSC_COUNT] = {0.0f};
    
    // Only recalculate when parameters change
    if (cached_cutoff[index] != cutoff || cached_dt[index] != dt) {
        float RC = 1.0f / (2.0f * M_PI * cutoff);
        cached_alpha[index] = dt / (RC + dt);
        cached_cutoff[index] = cutoff;
        cached_dt[index] = dt;
    }
    
    float alpha = cached_alpha[index];
...
static float _synth_highpass_buf_in[2 + OSC_COUNT] = {0};
static float _synth_highpass_buf_out[2 + OSC_COUNT] = {0};

static inline float synth_highpass(int index, float input, float cutoff, float dt) {
